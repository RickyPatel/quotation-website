...
class Buy extends Component {
  ...
  componentDidMount() {
    axios.get("/admin/getcategories").then((res) => {
      this.setState({
        categories: res.data,
      });
    });
  ...
  submitHandler = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        let k = items.length;
        for (let i = 0; i < k / 2; i++) temp.push(i);
        this.setState({
          items,
          cnt: temp,
        });
  ...
  render() {
    ...
    categories.unshift(<option key="0">Any</option>);
    let items = this.state.cnt.map((idx, index) => {
      return (
        <div className="row" key={index}>
          <div className="col-sm-6">
            <Item
              key={this.state.items[idx * 2]._id}
              item={this.state.items[idx * 2]}
              user={this.props.user}
            />
          </div>
          {idx * 2 + 1 < this.state.items.length ? (
            <div className="col-sm-6">
              <Item
                key={this.state.items[idx * 2 + 1]._id}
                item={this.state.items[idx * 2 + 1]}
                user={this.props.user}
              />
            </div>
          ) : (
      ...
      items = (
        <div className="container">
          <h4 style={{ fontSize: "100%" }}>
            No results found{" "}
            <img src="https://img.icons8.com/ultraviolet/40/000000/drama.png"></img>
          </h4>
        ...
        <div className="container" style={{ padding: "10px" }}>
          <form onSubmit={this.submitHandler}>
            <div className="form-group">
              ...
              <select className="form-control">{categories}</select>
            </div>
          ...
          <div style={{ marginBottom: "20px" }}>
            <h3 className="display-4" style={{ fontSize: "150%" }}>
              <label className="srch">
                <img src="https://img.icons8.com/dusk/64/000000/test-passed.png"></img>
                Search Results:
              </label>
            </h3>
          </div>
