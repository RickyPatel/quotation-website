...
class HomeNav extends Component {
  ...
  getNotificationText = (notification) => {
    if (notification.type === "INTEREST") {
      return (
        <Link
          to={{ pathname: "/sell", state: { key: 2 } }}
          className="notification-element"
        >
          <div className="notification-wrapper">
            <strong>{notification.sourceUsername}</strong> is interested in your
            item <strong>{notification.productName}</strong>
            <br />
            <small className="text-muted">
              {this.calcTime(notification.timeStamp)}
            </small>
          </div>
        </Link>
      );
      ...
      return (
        <Link to="" className="notification-element">
          <div className="notification-wrapper">
            <strong>{notification.sourceUsername}</strong> commented on your
            item <strong>{notification.productName}</strong> <br />
            <small className="text-muted">
              {this.calcTime(notification.timeStamp)}
            </small>
          </div>
      ...
      return (
        <Link
          to={{ pathname: "/user", state: { key: 4 } }}
          className="notification-element"
        >
          <div className="notification-wrapper" onClick={this.directUser}>
            <strong>{notification.sourceUsername + "'s"}</strong> item{" "}
            <strong>{notification.productName}</strong> is now{" "}
            <strong>{notification.payload.status.toLowerCase()} </strong> <br />
            <small className="text-muted">
              {this.calcTime(notification.timeStamp)}
            </small>
          </div>
      ...
      return (
        ...
        >
          <div className="notification-wrapper" onClick={this.directUser}>
            <strong>{notification.sourceUsername}</strong> has shared contact{" "}
            <br />
            <small className="text-muted">
              {this.calcTime(notification.timeStamp)}
            </small>
    ...
    return (
      <nav className="navbar navbar-expand-sm bg-dark navbar-dark sticky-top">
        ...
        <img
          ...
          alt="brand-logo"
        />
        <div className="logoname">Buy&Sell</div>
        <div className="collapse navbar-collapse" id="navbarToggler">
          <ul className="navbar-nav homenav me-auto mb-2 mb-lg-0">
            <li className={"nav-item " + this.state.status[0]}>
              <Link
                to={{ pathname: "/", state: { key: 0 } }}
                className="nav-link"
              >
                Home
              </Link>
            ...
            <li className={"nav-item " + this.state.status[3]}>
              <Link
                to={{ pathname: "/requirements", state: { key: 3 } }}
                className="nav-link"
              >
                Requirements
              </Link>
            ...
            <li className="nav-item">
              <a className="nav-link" href="#top" onClick={this.logOut}>
                Logout
              </a>{" "}
              {/* @debug: Float this to right */}
            </li>
            <li>
              <div className="nav-item dropdown">
                ...
                >
                  <img
                    ...
                    alt="bell-icon"
                  />
                  ({this.state.nofNotifications})
                </button>
                <div className="dropdown-menu notification-dropdown">
                  <h6 className="dropdown-header notification-item">
                    Notifications
                  </h6>
                  {notifications}
                </div>
