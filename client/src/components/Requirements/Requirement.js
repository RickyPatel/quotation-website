...
class Requirement extends Component {
  ...
  render() {
    ...
    let y = [];
    for (let i = 0; i < x.length / 2; i++) y.push(i);
    let t =
                  ...
                  <div className="col-sm-6">
                    <div className="card req-card">
                      <div className="card-body req-card-body">
                        <div key={index}>
                          <div className="card-title req-card-title">
                            <strong>{x[index * 2].title}</strong>
                          </div>
                          <div className="card-text req-card-text username">
                            {x[index * 2].username}
                          </div>
                          <div className="card-text req-card-text desc">
                            {x[index * 2].desc}
                          </div>
                          <div className="card-text req-card-text time">
                            <small className="text-muted">
                              {this.calcTime(x[index * 2].timestamp)}
                            </small>
                          </div>

                          <button
                            type="button"
                            className="btn btn-default comment-btn"
                            data-toggle="modal"
                            data-target={"#commentsModal" + x[index * 2]._id}
                          >
                            <img
                          ...
                          <div
                            ...
                            id={"commentsModal" + x[index * 2]._id}
                            tabIndex="-1"
                            role="dialog"
                    ...
                    <div className="col-sm-6">
                      <div className="card req-card">
                        <div className="card-body req-card-body">
                          <div key={index}>
                            <div className="card-title req-card-title">
                              <strong>{x[index * 2 + 1].title}</strong>
                            </div>
                            <div className="card-text req-card-text username">
                              {x[index * 2 + 1].username}
                            </div>
                            <div className="card-text req-card-text desc">
                              {x[index * 2 + 1].desc}
                            </div>
                            <div className="card-text req-card-text time">
                              <small className="text-muted">
                                {this.calcTime(x[index * 2 + 1].timestamp)}
                              </small>
                            </div>
                            ...
                            <div
                              className="modal interested-users-modal fade"
                              id={"commentsModal" + x[index * 2 + 1]._id}
                              tabIndex="-1"
                              role="dialog"
                              aria-labelledby="commentModalLabel"
                              aria-hidden="true"
