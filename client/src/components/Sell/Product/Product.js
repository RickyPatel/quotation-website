...
class Product extends Component {
  ...
  componentDidMount() {
    let usersList =
      this.props.item.interestedUsers.length > 0 ? (
  ...
  updateStatus = (status, id) => {
    ...
    axios.post("/notify/interestedUsers", notifications).then((res) => {});
  };
  ...
  render() {
    ...
    const handleDelete = (e) => {
      let confirmation = window.confirm(
        "This action will permanently delete this item. Do you want to continue?"
      );
      if (!confirmation) return;
      axios.post("/removeitem", this.props.item).then((res) => {
              ...
              <div className="container desc-list">
...

                {
                  <Dropdown
                    ...
                    id={this.props.id}
                    current={this.props.item.status}
                  />
                }
                ...
                <div className="bttns">
                  <dl className="row">
                    <dt className="col-sm-0">
                      ...
                      >
                        <img
                          src="https://img.icons8.com/material-outlined/24/000000/waste.png"
                          alt="waste"
                        />
                      </button>
                    </dt>
                    ...
                    <div className="card-text int-card-text time">
                      <small className="text-muted">
                        {this.calcTime(this.props.item.timestamp)}
                      </small>
                    </div>
                    <dt className="col-sm-3">
                      <button
                        type="button"
                        className="btn btn-default"
                        onClick={getInterestedUsers}
                        data-toggle="modal"
                        data-target={"#exampleModal" + this.props.item._id}
                      >
                        <img
                          src="https://img.icons8.com/ios-glyphs/24/000000/visible.png"
                          alt="visible-eye"
                        />
                      </button>
                    </dt>
                  ...
                  <div
                    ...
                    id={"exampleModal" + this.props.item._id}
                    tabIndex="-1"
                    role="dialog"
                      ...
                      <div className="modal-content">
                        <div className="modal-header">
                          ...
                          <button
                            ...
                            aria-label="Close"
                          >
                            <span aria-hidden="true" className="cross-btn">
                              &times;
                            </span>
                          </button>
                        </div>
                        <div className="modal-body interestBody">
                          {this.state.open ? this.state.usersList : ""}
                        </div>
