...
router.post("/verifyuser", (req, res) => {
  User.findOne({ username: req.body.username }).then(function (result) {
    if (result)
      req
        .check("username", "User already exists")
        .not()
        .equals(result.username);
...
router.post("/updatepassword", (req, res) => {
  User.findOneAndUpdate(
    { username: req.body.prevdetails.username },
    { ...req.body.prevdetails, password: req.body.newpassword }
  )
    .then((result) => {
      console.log(result);
      res.send("ok");
...
router.post("/updateitem", (req, res) => {
  Product.findOneAndUpdate({ _id: req.body.id }, { ...req.body.form }).then(
    (result) => {
      res.send("ok");
    }
  );
...
router.post("/shareStatus", (req, res) => {
  Product.findOneAndUpdate(
    ...
    {
      $set: { "interestedUsers.$.status": req.body.status },
    }
  ).then((result) => {
    res.send("ok");
  });
